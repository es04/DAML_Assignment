...
class PPOLoggerCallback(BaseCallback):
    """Captures PPO training metrics (rollout + train) into a single CSV."""

    def __init__(self, save_path="outputs/ppo_training_log.csv", verbose=0):
    ...
    def _on_rollout_end(self) -> None:
        ...
        for key, values in self.logger.name_to_value.items():
            # Flatten nested keys: rollout/ep_rew_mean → ep_rew_mean
    ...
    def _on_training_end(self) -> None:
        if self._rows:
            pd.DataFrame(self._rows).to_csv(self.save_path, index=False)
            print(f"  PPO training log → {self.save_path}")
...
def train_ppo_scheduler(total_timesteps=500_000, n_envs=4):
    ...
    log_cb = PPOLoggerCallback(save_path="outputs/ppo_training_log.csv")
...
if __name__ == "__main__":
    ...
    # Save unified Excel report — single sheet combining all results
    clf_df = pd.DataFrame(clf_results).set_index("model")
    clf_df.insert(0, "pipeline", "Classification")

    rul_df = pd.DataFrame(rul_results).set_index("model")
    rul_df.insert(0, "pipeline", "RUL_Prediction")

    sched_df = sched_results.copy()
    sched_df.insert(0, "pipeline", "Scheduling")

    combined_df = pd.concat([clf_df, rul_df, sched_df], axis=0, sort=False)
    combined_df.to_excel("outputs/full_pipeline_results.xlsx", sheet_name="All Results")
